(function (root, factory) {
  ...
  if (!configApi || !configApi.SUBJECTS || !configApi.SUBJECTS.math) {
    throw new Error("Matematik KS2 requires the shared subject configuration");
  }
  if (!slotSource || [1, 2, 3, 4, 5].some(function (slot) { return !Array.isArray(slotSource[slot]); })) {
    throw new Error("Matematik KS2 requires all five question slot scripts before questions.js");
  }

  return {
    config: configApi.SUBJECTS.math,
    slots: {
      1: slotSource[1],
      2: slotSource[2],
      3: slotSource[3],
